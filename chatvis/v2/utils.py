"""Extraction helpers ported verbatim from ``Utility/extract_utils.py``.

The upstream behavior is preserved exactly, including the known bug in
``extract_python_code`` (``return filename`` inside the ``for`` loop, so only
the first python block is ever written) and the traceback-only matching in
``extract_error_messages`` (no silent-failure allowlist).
"""

...
# read the input data
velocity = IOSSReader(
    registrationName="disk.ex2",
    FileName=["/Users/tpeterka/collaborations/tanwi/examples/stream-glyph/disk.ex2"],
)
...
streamTracer = StreamTracer(
    registrationName="StreamTracer1", Input=velocity, SeedType="Point Cloud"
)
...
renderView.CameraPosition = [
    (bounds[0] + bounds[1]) / 2.0 - 1.5 * max(length[1], length[2]),
...
SaveScreenshot(
    "/Users/tpeterka/collaborations/tanwi/examples/stream-glyph/stream-glyph-screenshot.png",
    renderView,
